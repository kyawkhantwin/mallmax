        ...
        <tbody>
           @foreach($products as  $key => $product)
               <tr>
                   <td>{{$key + 1 }}</td>
                   <td>
                       <img src="{{$product -> image_url}}" class="img-thumbnail" width="70" alt="">
                   </td>
                   <td>{{$product -> name}}</td>
                   <td>
                       <form action="{{url('admin/product/'.$product->slug.'/edit')}}" method="GET" class="d-inline">
                           @csrf
                           <button class="btn btn-success">Edit</button>
                       </form>
                       <form action="{{url('admin/product/'.$product->slug)}}" method="POST" class="d-inline">
                           @csrf
                           @method('DELETE')
